...
def timed_step(step_name, func, *args, **kwargs):
    """Utility to time each pipeline step."""
...
def run_pipeline(video_path, results_base_dir, use_stab=False, use_seg=False):
    ...
    if not video_5fps.exists():
        video_5fps = timed_step("5fps conversion", convert_to_5fps, video_path, video_5fps)
    else:
    ...
    if use_stab:
        ...
        if not stabilized_video.exists():
            timed_step("Stabilization", stabilize_video, video_5fps, stabilized_video)
    ...
    if use_seg:
        ...
        timed_step("Segmentation", segment_video, processed_video, str(segment_dir))
    ...
    print(f"Running detections on {len(video_inputs)} file(s) for {video_path.name}...")
    for v in tqdm(video_inputs, desc=f"Detections [{video_path.stem}]", unit="video"):
        face_dir = version_dir / "detections" / v.stem / "face"
        obj_dir = version_dir / "detections" / v.stem / "object"
        face_csv = timed_step("Face detection", face_detect, str(v), str(face_dir))
        face_csv_paths.append(face_csv)
        object_csv = timed_step("Object detection", object_detect, str(v), str(obj_dir))
    ...
    timed_step("Merging CSVs", merge_csvs, face_csv_paths, object_csv_paths, merged_csv_path, video_path.stem)
...
def main():
    ...
    for idx, video_path in enumerate(video_files, start=1):
        print(f"\n=== Processing video {idx}/{len(video_files)}: {video_path.name} ===")
        ...
        print(f"=== Finished {video_path.name} in {elapsed_video/60:.2f} minutes ===\n")
